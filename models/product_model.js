...
var product = {
    ...
    UpdateProduct: function (pro_id,item,callback) {
        return db.query('update product_tbl set pro_name=?,info=?,size=?,color=?,rent=?,soh=?,fashion=?,sp_instruction=?,fk_cat_id=? where pro_id=?',
        [item.pro_name, item.info, item.size, item.color, item.rent, item.soh, item.fashion,item.sp_instruction, item.fk_cat_id,pro_id], callback);
    },
    EditProduct:function(pro_id,item,callback)
    {
        console.log(soh);
        console.log("product update model");
        return db.query('update product_tbl set soh=? where pro_id=?',[item.soh,pro_id],callback);
    }
};
