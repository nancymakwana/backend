...
var product = {
    ...
    addProduct: function (item,callback) {
        return db.query('insert into product_tbl  values(?,?,?,?,?,?,?,?,?)', [item.pro_id, item.pro_name, item.info, item.size, item.color, item.rent, item.soh, item.sp_instruction, item.fk_cat_id], callback);
    },
