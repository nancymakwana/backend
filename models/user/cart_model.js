...
var cart = {
    ...
    return db.query('select c.*,u.user_id,p.*,i.* from product_tbl p join pro_img_tbl i join cart_tbl c join user_tbl u on p.pro_id=i.fk_pro_id and u.user_id=c.fk_user_id and c.fk_pro_id = p.pro_id  and fk_user_id= ? GROUP BY i.fk_pro_id',[user_id],callback);
    },
    deleteCartItems: function (cart_id, callback) {
        return db.query('delete from cart_tbl where cart_id=?', [cart_id], callback);
    },
    deleteCart: function (user_id, callback) {
        return db.query('delete from cart_tbl where fk_user_id=?', [user_id], callback);
    },
