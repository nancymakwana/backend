...
addUser:function(item,callback){
    return db.query('insert into user_tbl values(?,?,?,?,?,?,?,?,?,?,?,?)',[item.user_id,item.user_email,item.password,item.fname,item.lname,item.address,item.city,item.pincode,item.mob_no,item.gender,item.img,item.type],callback);
},
